  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 -z-30 h-full w-full overflow-hidden">
        ...
        <img
          className={`absolute -top-[2.5%] -left-[12.5%] lg:-top-[10%] lg:-left-[25%] ${window.innerWidth < 1024 ? "w-full" : "h-full"} border border-red-500`}
          src={`${assetsUrl.iconsUrl}sharpe1.svg`}
        />
        <img
          className={`absolute -right-[12.5%] -bottom-[2.5%] lg:-right-[25%] lg:-bottom-[10%] ${window.innerWidth < 1024 ? "w-full" : "h-full"} shrink-0 rotate-z-180 border border-red-500`}
          src={`${assetsUrl.iconsUrl}sharpe1.svg`}
        />
      </div>

      {/* text */}
      <div className="w-[85%] md:w-[75%] lg:w-[65%] 2xl:w-[60%]">
        <img className="w-full" src={`${assetsUrl.imagesUrl}AboutUsText.png`} />
      </div>
    </section>
  );
};
